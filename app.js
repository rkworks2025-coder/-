...
var Junkai = (() => {
  ...
  function loadLocalConfig() {
    ...
    if (cached) { try { appConfig = Array.isArray(JSON.parse(cached)) ? JSON.parse(cached) : []; } catch(e) { appConfig = []; } } 
    else { appConfig = []; }
  }
  ...
  function getDefaultFilter() { return { standby: true, stop: true, skip: false, "7days_rule": false, checked: false }; }
  function loadFilter(city) { try { const saved = localStorage.getItem(LS_FILTER_KEY(city)); if (saved) return JSON.parse(saved); } catch(e) {} return getDefaultFilter(); }
  function saveFilter(city, filter) { localStorage.setItem(LS_FILTER_KEY(city), JSON.stringify(filter)); }
  ...
  function renderIndexButtons() {
    ...
    appConfig.forEach(cfg => {
      ...
      meta.className = "meta";
      meta.innerHTML = `<span class="chip">済 <span id="${slug}-done">0</span></span><span class="chip">停 <span id="${slug}-stop">0</span></span><span class="chip">不要 <span id="${slug}-skip">0</span></span><span class="chip">総 <span id="${slug}-total">0</span></span><span class="chip">残 <span id="${slug}-rem">0</span></span>`;
      a.appendChild(h2); a.appendChild(meta); container.appendChild(a);
    });
  }

  function saveCity(city, arr) { localStorage.setItem(LS_KEY(city), JSON.stringify(arr)); }
  function readCity(city) { try { const s = localStorage.getItem(LS_KEY(city)); if (!s) return []; const a = JSON.parse(s); return Array.isArray(a) ? a : []; } catch (_) { return []; } }

  function applyUIIndex(city, arr) {
    const target = appConfig.find(c => c.name === city);
    const p = target ? target.prefix : "?";
    for (let i = 0; i < arr.length; i++) { arr[i].ui_index_num = i + 1; arr[i].ui_index = p + (i + 1); }
  }

  function normalizeRow(rowObj) {
    return {
      area: (rowObj.area || "").trim(), city: (rowObj.city || "").trim(), address: (rowObj.address || "").trim(), station: (rowObj.station || "").trim(),
      model: (rowObj.model || "").trim(), plate: (rowObj.plate || "").trim(), note: (rowObj.note || "").trim(), operator: (rowObj.operator || "").trim(),
      status: (rowObj.status || "").trim(), checked: !!rowObj.checked, last_inspected_at: (rowObj.last_inspected_at || "").trim(),
      index: Number.isFinite(+rowObj.index) ? parseInt(rowObj.index, 10) : 0, ui_index: rowObj.ui_index || "", ui_index_num: rowObj.ui_index_num || 0
    };
  }

  function countCity(arr) {
    const c = { done: 0, stop: 0, skip: 0, total: arr.length };
    for (const it of arr) { if (it.status === "stop") c.stop++; else if (it.status === "skip") c.skip++; if (it.checked) c.done++; }
    return c;
  }
  ...
  function repaintCounters() {
    ...
    appConfig.forEach(cfg => {
      const city = cfg.name; const slug = cfg.slug; const arr = readCity(city); const cnt = countCity(arr);
      ...
      if(document.getElementById(`${slug}-done`)) {
        document.getElementById(`${slug}-done`).textContent = cnt.done; document.getElementById(`${slug}-stop`).textContent = cnt.stop;
        document.getElementById(`${slug}-skip`).textContent = cnt.skip; document.getElementById(`${slug}-total`).textContent = cnt.total;
        document.getElementById(`${slug}-rem`).textContent = (cnt.total - cnt.done - cnt.skip);
      }
    });
    const updateEl = (id, val) => { const el = document.querySelector(id); if (el) el.textContent = val; };
    updateEl("#all-done", overallDone); updateEl("#all-stop", overallStop); updateEl("#all-skip", overallSkip);
    updateEl("#all-total", overallTotal); updateEl("#all-rem", (overallTotal - overallDone - overallSkip));
  ...
  async function execPullLog() {
    ...
    if (!ok) return;
    try {
      showProgress(true, 10); statusText("ログを取得中...");
      ...
      statusText("データ反映中...");
      const logRows = json.rows; let updatedCount = 0, addedCount = 0, deletedCount = 0; 
      for (const cfg of appConfig) {
        let cityData = readCity(cfg.name); let isCityModified = false;
        const cityLogs = logRows.filter(r => r.city === cfg.name); const validPlates = cityLogs.map(r => r.plate);
        const preCount = cityData.length;
        cityData = cityData.filter(localRow => validPlates.includes(localRow.plate));
        if (preCount !== cityData.length) { deletedCount += (preCount - cityData.length); isCityModified = true; }
        cityLogs.forEach(logRow => {
          ...
          if (targetRow) {
            if (targetRow.checked !== newChecked || targetRow.status !== newStatus || targetRow.last_inspected_at !== newDate) {
                targetRow.checked = newChecked; targetRow.status = newStatus; targetRow.last_inspected_at = newDate; isCityModified = true; updatedCount++;
            }
          } else {
            cityData.push(normalizeRow({ city: cfg.name, station: logRow.station, model: logRow.model, plate: logRow.plate, note: "", operator:"", status: newStatus, checked: newChecked, last_inspected_at: newDate, ui_index: logRow.ui_index || "", ui_index_num: 999 }));
            isCityModified = true; addedCount++;
          }
        });
        if (isCityModified) { applyUIIndex(cfg.name, cityData); saveCity(cfg.name, cityData); }
      }
      repaintCounters(); showProgress(true, 100); statusText(`Pull完了 (更新:${updatedCount}, 追加:${addedCount}, 削除:${deletedCount})`);
  ...
  async function initIndex() {
    ...
    if (btn) {
      btn.addEventListener("click", async () => {
        ...
        try {
          showProgress(true, 5); statusText("設定ファイル更新中…");
          ...
          for (const r of json.rows) { const norm = normalizeRow(r); if (buckets[norm.city]) buckets[norm.city].push(norm); }
          for (const cfg of appConfig) {
            const arr = buckets[cfg.name];
            if (arr && arr.length > 0) { applyUIIndex(cfg.name, arr); saveCity(cfg.name, arr); }
          }
          renderIndexButtons(); repaintCounters(); showProgress(true, 100); statusText("同期完了");
        } catch (e) { statusText("同期失敗：" + e.message); } finally { setTimeout(() => showProgress(false), 400); }
      });
    }
    const pullBtn = document.getElementById("pushLogBtn");
    if (pullBtn) { pullBtn.textContent = "Pull"; pullBtn.addEventListener("click", execPullLog); }
  }

  async function syncInspectionAll() {
    const all = []; appConfig.forEach(cfg => all.push(...readCity(cfg.name)));
    try {
      const h=document.getElementById("hint"); if(h) h.textContent="送信中...";
      await fetch(`${GAS_URL}?action=syncInspection`, { method: "POST", body: JSON.stringify({ data: all }) });
      if(h) { h.textContent="送信成功"; setTimeout(()=>h.textContent=`件数：${all.length}`, 1500); }
    } catch (e) { if(document.getElementById("hint")) document.getElementById("hint").textContent="送信失敗"; }
  }
  ...
  function persistCityRec(city, rec) {
    ...
    if (idx === -1) return;
    arr[idx] = rec; saveCity(city, arr); repaintCounters();
  }
  ...
  async function initCity(cityKey) {
    ...
    if (!targetCfg) { if(document.getElementById("hint")) document.getElementById("hint").textContent = "設定エラー"; return; } 
    else { cityName = targetCfg.name; }
    
    if (document.getElementById("pageTitle")) document.getElementById("pageTitle").textContent = targetCfg.name;
    if (document.getElementById("headerTitle")) document.getElementById("headerTitle").textContent = targetCfg.name;
    ...
    function updateFilterButton() { if (filterBtn) filterBtn.textContent = `フィルタ: ${getFilterLabel(currentFilter)} ▼`; }
    ...
    function renderList() {
      ...
      for (const rec of filteredArr) {
        ...
        const topLeft = document.createElement("div"); topLeft.className = "left-top";
        ...
        if (rec.last_inspected_at) {
          ...
          dtDiv.textContent = dispDate; left.appendChild(dtDiv);
        }
        
        chk.addEventListener("change", (e) => {
          const checkModal = document.getElementById('checkModal');
          if (!checkModal) {
            if (!confirm(`【${rec.plate}】\n${chk.checked ? "チェックしますか?" : "外しますか?"}`)) { chk.checked = !chk.checked; return; }
            executeCheck(); return;
          }
          document.getElementById('checkModalTitle').textContent = `【${rec.plate}】`;
          if (document.getElementById('checkModalModel')) document.getElementById('checkModalModel').textContent = rec.model || "";
          if (document.getElementById('checkModalMsg')) document.getElementById('checkModalMsg').textContent = chk.checked ? "チェックしますか？" : "外しますか？";
          checkModal.classList.add('show');
          document.getElementById('checkModalOk').onclick = () => { checkModal.classList.remove('show'); executeCheck(); };
          document.getElementById('checkModalCancel').onclick = () => { checkModal.classList.remove('show'); chk.checked = !chk.checked; };

          function executeCheck() {
            rec.checked = chk.checked; rec.last_inspected_at = chk.checked ? getTodayJST() : "";
            row.className = `row ${rowBg(rec)}`; persistCityRec(cityName, rec); syncInspectionAll(); renderList(); 
          }
        });
        ...
        tmaBtn.addEventListener("click", () => {
          ...
          document.getElementById('tmaModalTitle').textContent = `【${rec.plate}】`;
          if (document.getElementById('tmaModalModel')) document.getElementById('tmaModalModel').textContent = rec.model || "";
          tmaModal.classList.add('show');
          document.getElementById('tmaModalOk').onclick = () => { tmaModal.classList.remove('show'); executeTma(); };
    ...
    if (filterBtn && filterModal) {
      filterBtn.addEventListener("click", () => {
        document.getElementById("filter_standby").checked = currentFilter.standby; document.getElementById("filter_stop").checked = currentFilter.stop;
        document.getElementById("filter_skip").checked = currentFilter.skip; document.getElementById("filter_7days").checked = currentFilter["7days_rule"];
    ...
    if (filterApply) {
      filterApply.addEventListener("click", () => {
        currentFilter.standby = document.getElementById("filter_standby").checked; currentFilter.stop = document.getElementById("filter_stop").checked;
        currentFilter.skip = document.getElementById("filter_skip").checked; currentFilter["7days_rule"] = document.getElementById("filter_7days").checked;
        currentFilter.checked = document.getElementById("filter_checked").checked;
        saveFilter(cityName, currentFilter); updateFilterButton(); filterModal.classList.remove("show"); renderList();
      });
    }

    if (document.getElementById("filterCancel")) document.getElementById("filterCancel").addEventListener("click", () => filterModal.classList.remove("show"));
    updateFilterButton(); renderList();
  }

  async function initAreaPage() {
    const params = new URLSearchParams(window.location.search);
    const cityKey = params.get('city');
    if (!cityKey) { if(document.getElementById("hint")) document.getElementById("hint").textContent = "対象エリア未指定"; return; }
