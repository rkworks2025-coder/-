...
const Junkai = (() => {
  ...
  btn.addEventListener("click", async () => {
    if (!confirm("全件を最新状態にリセットして同期します。よろしいですか？")) return;
    ...
    try {
      ...
      // 1) 初期化リクエスト
      const url = `${GAS_URL}?action=init&_=${Date.now()}`;
  ...
  function updateDateTime() {
    // dummy（もともとこのファイルにある実装をそのまま残している想定）
  }
